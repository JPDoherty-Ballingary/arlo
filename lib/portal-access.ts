 ...
 *
 * Recipients have no password — this link *is* their login. Requesting a new
 * one is therefore a "reset access" action: any links issued previously for
 * this recipient are invalidated first, so a lost or leaked link stops
 * working the moment a new one is requested.
 */
export async function issuePortalAccessLink(params: {
  recipientId: string
  ownerId: string
  email: string
}): Promise<{ ok: true } | { ok: false; reason: 'insert_failed' | 'send_failed' }> {
  const { recipientId, ownerId, email } = params

  // Reset: revoke any links already issued for this recipient before
  // creating the new one.
  await supabaseAdmin.from('recipient_sessions').delete().eq('recipient_id', recipientId)
  ...
  if (insertError) {
    console.error('recipient_sessions insert failed', insertError)
...
<div style="max-width:600px;margin:0 auto;background:#ffffff;">
  ...
  if (sendError) {
    console.error('Failed to send portal access link', sendError)
